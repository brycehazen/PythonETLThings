...
def process_csv(input_dir: Path, output_dir: Path) -> None:
   # Loop over all CSV files in the input directory
    ...
    # Change State column
        ...
        output_dir_name = f"{csv_file.stem}_output"

        # Create the output directory if it doesn't exist
        output_dir_path = output_dir / output_dir_name
        output_dir_path.mkdir(exist_ok=True)

        # Create the ReFiles directory if it doesn't exist
        re_dir_path = output_dir_path / "ReFiles"
        re_dir_path.mkdir(exist_ok=True)
        ...
        failed_path = output_dir_path / failed_csv
        dup_path = output_dir_path / dup_csv
        renew_path = re_dir_path / renew_csv
        passed_path = output_dir_path / passed_csv
        import_path = re_dir_path / import_csv
        rawdata_path = re_dir_path / rawdata_csv
        ...
        newimpomatic.to_csv(import_path, index=False)
        rawdata.to_csv(rawdata_path, index=False)
...
def main() -> None:
    ...
    # Create a function to handle the input directory selection
    def select_input_dir():
        input_dir = filedialog.askdirectory(title="Select input directory")
        input_dir_entry.delete(0, tk.END)
        input_dir_entry.insert(0, input_dir)

    # Create a function to handle the output directory selection
    def select_output_dir():
        output_dir = filedialog.askdirectory(title="Select output directory")
        output_dir_entry.delete(0, tk.END)
        output_dir_entry.insert(0, output_dir)

    # Create the input directory selection button
    input_dir_button = tk.Button(root, text="Select input directory", command=select_input_dir)
    input_dir_button.grid(row=0, column=0)
    ...
    output_dir_button.grid(row=1, column=0)
    ...
    process_button = tk.Button(root, text="Process CSV files", command=lambda: process(Path(input_dir_entry.get()), Path(output_dir_entry.get())))
